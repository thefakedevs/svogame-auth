...
function normalizeDynamicPathname(pathname: string) {
  return pathname
    ...
    .replace(
      ...
      "/admin/users/:userId/lootboxes/open-history",
    )
    .replace(/^\/admin\/users\/[^/]+\/littlemice$/, "/admin/users/:userId/littlemice")
    .replace(
      /^\/admin\/users\/[^/]+\/littlemice\/[^/]+$/,
      "/admin/users/:userId/littlemice/:checkId",
    )
...
function clickEventName(element: Element) {
  const explicitName =
    element.getAttribute("data-analytics-event") || element.getAttribute("data-umami-event");
...
function onDocumentSubmit(event: SubmitEvent) {
  ...
  const eventName =
    ...
    form.getAttribute("data-umami-event") ||
    "form_submit";
