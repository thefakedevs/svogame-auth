...
export function getMyShopOrderReceipt(
  token: string,
  orderId: string,
): Promise<ShopReceiptResponse> {
  return request<ShopReceiptResponse>(
    `/api/user/me/shop/orders/${encodeURIComponent(orderId)}/receipt`,
    {
      headers: authHeaders(token, {
        "Content-Type": "application/json",
      }),
