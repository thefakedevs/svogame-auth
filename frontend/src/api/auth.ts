...
export async function fetchAuthorize(
  ...
  if (!pow) {
    throw new Error(
      "Р›РѕРєР°Р»СЊРЅР°СЏ СЃРµСЃСЃРёСЏ Р°РІС‚РѕСЂРёР·Р°С†РёРё СѓСЃС‚Р°СЂРµР»Р°. РќР°С‡РЅРёС‚Рµ РІС…РѕРґ Р·Р°РЅРѕРІРѕ.",
    );
  }

  const data = await request<RawAuthorizedAuthResponse | RawTermsRequiredAuthResponse>(
    "/api/auth/authorize",
    {
      method: "POST",
      headers: {
