...
export function revokeAdminUserSuperuser(
  token: string,
  userId: string,
...
export function grantAdminUserRestriction(
  ...
  return request<AdminUserRestrictionResponse[]>(
    `/api/admin/users/${userId}/restrictions/${encodeURIComponent(restrictionKey)}`,
    {
      method: "POST",
      headers: authHeaders(token, {
        "Content-Type": "application/json",
...
export function getAdminUserSquad(
  token: string,
  userId: string,
): Promise<AdminSquadResponse | null> {
...
export async function listAllAdminSquads(
  token: string,
  perPage = 100,
...
export function uploadAdminSquadImage(
  token: string,
  squadId: string,
  file: File,
): Promise<AdminSquadResponse> {
...
export function kickAdminSquadMember(
  token: string,
  squadId: string,
  userId: string,
...
export function listAdminShopProducts(
  token: string,
  locale?: string | null,
...
export function listAdminShopOrders(
  ...
  if (query?.perPage) params.set("perPage", String(query.perPage));
  const suffix = params.toString();
  return request<AdminShopOrdersListResponse>(
    `/api/admin/shop/orders${suffix ? `?${suffix}` : ""}`,
    {
      headers: authHeaders(token, {
        "Content-Type": "application/json",
      }),
...
export function createAdminShopProduct(
  token: string,
  body: CreateShopProductInput,
): Promise<ShopProductResponse> {
...
export function getAdminShopProduct(
  token: string,
  productId: string,
  locale?: string | null,
): Promise<ShopProductResponse> {
