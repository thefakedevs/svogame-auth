...
export default function ProfileSettingsTab({
  ...
  const onNicknameChange = (value: string) => {
    ...
    setHasInvalidNicknameInput(
      sanitizedValue !== value || isInvalidNicknameLength(sanitizedValue.trim()),
    );
    ...
    <div className="profile-split">
      <section className="card profile-panel">
        <div className="ui-card-header">
          <h2 className="card-title">Данные аккаунта</h2>
        </div>
        <div className="profile-form">
          <div className={`ui-field ${hasInvalidNicknameInput ? "ui-field-error" : ""}`}>
            <label className="ui-label" htmlFor="profile-nickname">
              Никнейм
            </label>
            <input
              id="profile-nickname"
          ...
          <div className="profile-actions">
            <button
              ...
              type="button"
              disabled={isUpdatingNickname || hasInvalidNicknameInput}
        ...
        <dl className="profile-kv profile-kv--wide">
          <div>
            <dt>E-mail</dt>
            <dd>{data.user.email ?? "Не указан"}</dd>
          </div>
          <div>
            <dt>Discord</dt>
          ...
          <div>
            ...
            <dd>{formatDate(data.user.createdAt).replace(", ", " ")}</dd>
          </div>
          <div>
      ...
      <section className="card profile-panel">
        <div className="ui-card-header">
          <h2 className="card-title">Скин и сессия</h2>
        </div>
