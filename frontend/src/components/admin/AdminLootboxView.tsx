...
function draftFromDrop(drop: LootboxDropResponse): DropDraft {
  return {
    ...
    duplicateCompensationAmount:
      drop.duplicateCompensationAmount != null ? String(drop.duplicateCompensationAmount) : "",
...
export default function AdminLootboxView({
  token,
  lootboxId,
}: {
  token: string;
  ...
  const load = async () => {
    ...
    try {
      ...
      setDropDrafts(
        Object.fromEntries(loadedDetail.drops.map((drop) => [drop.id, draftFromDrop(drop)])),
      );
    } catch (cause) {
  ...
  const buildDropPatch = (draft: DropDraft, assetKey: string) => {
    ...
    if (asset.ownershipModel === "stackable") {
      ...
      if (!Number.isFinite(amount) || amount <= 0)
        throw new Error("amount должен быть больше 0 для stackable.");
      return {
        amount,
    ...
    if (asset.ownershipModel !== "expirable")
      throw new Error(
        "API лутбоксов поддерживает только stackable, expirable и entitlement rewards.",
      );
  ...
  const saveDrop = async (drop: LootboxDropResponse) => {
    ...
    try {
      const updated = await patchAdminLootboxDrop(
        token,
        detail.definition.id,
        drop.id,
        buildDropPatch(draft, drop.rewardAssetKey),
      );
      setDetail(updated);
      setDropDrafts(
        Object.fromEntries(updated.drops.map((item) => [item.id, draftFromDrop(item)])),
      );
          ...
          <section className="card admin-card">
            <div className="admin-section-head admin-detail-hero">
              <div>
                ...
                <p className="card-text">
                  <code>{detail.definition.assetKey}</code> · ID:{" "}
                  <code>{detail.definition.id}</code>
                </p>
              </div>
              <span
                className={`ui-badge ${detail.definition.isActive ? "ui-badge-success" : "ui-badge-warning"}`}
              >
            ...
            <dl className="admin-kv admin-kv--compact">
              <div>
                ...
                <dd>{detail.definition.assetDefinitionId}</dd>
              </div>
              <div>
                <dt>Видимость лутбокса</dt>
                <dd>{detail.definition.isPublic ? "public" : "hidden"}</dd>
              </div>
              <div>
                <dt>Drops</dt>
                <dd>{detail.drops.length}</dd>
              </div>
            </dl>
                    ...
                    {asset && draft ? (
                      <>
                        <DynamicDropValueField
                          asset={asset}
                          draft={draft}
                          onChange={(patch) => updateDropDraft(drop.id, patch)}
                        />
                        <DuplicateCompensationField
                          value={draft.duplicateCompensationAmount}
                          onChange={(duplicateCompensationAmount) =>
                            updateDropDraft(drop.id, { duplicateCompensationAmount })
                          }
                        />
                        <label className="admin-lootbox-field">
                          <input
                            ...
                            value={draft.weight}
                            onChange={(event) =>
                              updateDropDraft(drop.id, { weight: event.target.value })
                            }
                          />
                          <small>weight</small>
                        </label>
                        <label className="admin-lootbox-field">
                          <input
                            ...
                            value={draft.title}
                            onChange={(event) =>
                              updateDropDraft(drop.id, { title: event.target.value })
                            }
                          />
                        ...
                        <div className="admin-lootbox-cell-actions">
                          <button
                            ...
                            disabled={mutatingDropId === drop.id}
                          ...
                          <button
                            ...
                            disabled={mutatingDropId === drop.id}
                            onClick={() => void deleteDrop(drop.id)}
                          >
                            Удалить
                          </button>
                        </div>
                      ...
                      <>
                        <span className="admin-inline-muted admin-lootbox-empty-column">
                          Ассет не найден
                        </span>
              ...
              <article className="admin-lootbox-cell admin-lootbox-cell--empty" role="row">
                ...
                <div className="admin-lootbox-cell-asset">
                  <AssetPicker
                    ...
                    onChange={(rewardAssetKey) =>
                      setNewDrop((prev) => ({ ...prev, rewardAssetKey }))
                    }
                  />
                ...
                {assetsByKey.get(newDrop.rewardAssetKey) ? (
                  <>
                    ...
                    <DuplicateCompensationField
                      ...
                      onChange={(duplicateCompensationAmount) =>
                        setNewDrop((prev) => ({ ...prev, duplicateCompensationAmount }))
                      }
                    />
                    <label className="admin-lootbox-field">
                      <input
                        ...
                        onChange={(event) =>
                          setNewDrop((prev) => ({ ...prev, weight: event.target.value }))
                        }
                      />
                      <small>weight</small>
                    </label>
                    <label className="admin-lootbox-field">
                      <input
                        ...
                        value={newDrop.title}
                        onChange={(event) =>
                          setNewDrop((prev) => ({ ...prev, title: event.target.value }))
                        }
                      />
                      <small>title_i18n ru-RU</small>
                    </label>
                    <div className="admin-lootbox-cell-actions">
                      <button
                        ...
                        disabled={mutatingDropId === "new"}
                        onClick={() => void addDrop()}
                      >
