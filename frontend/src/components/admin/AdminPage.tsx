...
export default function AdminPage() {
  ...
  if (route.type === "userLittlemice") {
    return (
      <AdminUserLittlemiceChecksView token={token!} userId={route.userId} checkId={route.checkId} />
    );
  }
  ...
  return (
    <AdminSquadProfile
      ...
      onMembersChange={setSquadMembers}
    />
  );
}
...
function AdminHome({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const [usersResponse, squadsResponse, serviceTokensResponse, skinResponse] =
          await Promise.all([
  ...
  const createToken = async () => {
    ...
    try {
      ...
      onTokensChange((prev) => [
        created,
        ...prev.filter((tokenItem) => tokenItem.id !== created.id),
      ]);
  ...
  const rotateToken = async (tokenId: string) => {
    try {
      ...
      onTokensChange((prev) =>
        prev.map((tokenItem) => (tokenItem.id === tokenId ? rotated : tokenItem)),
      );
  ...
  const revokeToken = async (tokenId: string) => {
    try {
      ...
      onTokensChange((prev) =>
        prev.map((tokenItem) => (tokenItem.id === tokenId ? revoked : tokenItem)),
      );
    ...
    <div className="admin-page">
      <section className="admin-top-actions">
        <nav className="admin-tabs">
          <AdminLink
            href={adminHomeTabPath("overview")}
            replace
          ...
          <AdminLink
            href={adminHomeTabPath("squads")}
            replace
            className={`admin-tab ${tab === "squads" ? "is-active" : ""}`}
          >
            Сквады
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("assets")}
            replace
            className={`admin-tab ${tab === "assets" ? "is-active" : ""}`}
          >
            Ассеты
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("timeline")}
            replace
            className={`admin-tab ${tab === "timeline" ? "is-active" : ""}`}
          >
            История покупок
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("shop")}
            replace
            className={`admin-tab ${tab === "shop" ? "is-active" : ""}`}
          >
            Магазин
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("skinShop")}
            replace
            className={`admin-tab ${tab === "skinShop" ? "is-active" : ""}`}
          >
            Скины → магазин
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("lootboxes")}
            replace
            className={`admin-tab ${tab === "lootboxes" ? "is-active" : ""}`}
          >
            Лутбоксы
          </AdminLink>
          <AdminLink href={adminLittlemicePath()} className="admin-tab">
            Littlemice
          </AdminLink>
          <AdminLink
            href={adminHomeTabPath("tokens")}
            replace
            className={`admin-tab ${tab === "tokens" ? "is-active" : ""}`}
          >
            Сервисные токены
          </AdminLink>
        </nav>
      ...
      {tab === "overview" ? (
        <section className="admin-overview">
          ...
          {!overviewStats && !overviewError ? (
            <LoadingState title="Загружаем обзор админки" />
          ) : null}
              ...
              <section className="card admin-card admin-default-skin-card">
                ...
                <div className="admin-default-skin-info">
                  ...
                  <dl className="admin-kv admin-kv--compact">
                    <div>
                      <dt>Формат</dt>
                      <dd>{defaultSkin?.contentType ?? "—"}</dd>
                    </div>
                    <div>
                      <dt>Обновлен</dt>
                      <dd>{formatDateTime(defaultSkin?.updatedAt)}</dd>
                    </div>
                    <div>
                      <dt>Администратор</dt>
                      <dd>{defaultSkin?.updatedByUserId ?? "—"}</dd>
                    </div>
                  </dl>
        ...
        <section className="card admin-card">
          <div className="admin-section-head">
            ...
            <span className="ui-badge ui-badge-neutral">
              Страница {usersPage} из {Math.max(1, usersTotalPages)}
            </span>
          </div>
          ...
          <div className="admin-list">
            {users.map((user) => (
              <AdminLink key={user.id} href={adminUserPath(user.id)} className="admin-row">
                <span className="admin-row-user">
                  {user.avatarUrl ? (
                    <img src={user.avatarUrl} alt={user.username} className="admin-avatar" />
                  ) : (
                    <span className="ui-avatar ui-avatar-sm">{initials(user.username)}</span>
                  )}
            ...
            {users.length === 0 ? (
              <p className="admin-inline-muted">Пользователи не найдены.</p>
            ) : null}
          </div>
          <AdminPagination
            page={usersPage}
            ...
            label="Нумерация страниц пользователей"
          />
        </section>
      ) : null}
      ...
      {tab === "squads" ? (
        <section className="card admin-card">
          <div className="admin-section-head">
            ...
            <span className="ui-badge ui-badge-neutral">
              Страница {squadsPage} из {Math.max(1, squadsTotalPages)}
            </span>
          </div>
          <div className="admin-metric-strip admin-metric-strip--compact">
            <div className="admin-metric">
          ...
          <div className="admin-filter-bar">
            <label className="admin-shop-field">
              ...
              <input
                className="ui-input"
                value={squadsQuery}
                onChange={(event) => onSquadsQueryChange(event.target.value)}
                placeholder="Название или ID сквада"
              />
            </label>
          </div>
          <div className="admin-list">
            {squads.map((squadItem) => (
              <AdminLink
                key={squadItem.id}
                href={adminSquadPath(squadItem.id)}
                className="admin-row"
              >
                <span className="admin-row-user">
                  {squadItem.imageUrl ? (
                    <img src={squadItem.imageUrl} alt={squadItem.name} className="admin-avatar" />
                  ) : (
                    <span className="ui-avatar ui-avatar-sm">{initials(squadItem.name)}</span>
                  )}
                  <span className="admin-row-user-text">
                ...
                <span className="admin-row-badges">
                  <span className="ui-badge ui-badge-neutral">
                    {squadItem.memberCount}/{squadItem.maxMembers}
                  </span>
                </span>
              </AdminLink>
            ))}
          ...
          <AdminPagination
            page={squadsPage}
            totalPages={Math.max(1, squadsTotalPages)}
            onPageChange={onSquadsPageChange}
            label="Нумерация страниц сквадов"
          />
        </section>
      ) : null}
          ...
          <div className="admin-token-create admin-filter-bar">
            <label className="admin-shop-field">
              ...
              <input
                className="ui-input"
                value={systemName}
                onChange={(event) => setSystemName(event.target.value.toLowerCase())}
                placeholder="discord-worker"
              />
            </label>
            ...
            {tokens.map((tokenItem) => (
              <article key={tokenItem.id} className="admin-token-card">
                <div className="admin-token-head">
                  ...
                  <span
                    className={`ui-badge ${tokenItem.isActive ? "ui-badge-success" : "ui-badge-warning"}`}
                  >
                ...
                <div className="admin-token-actions">
                  <button
                    ...
                    onClick={() => void rotateToken(tokenItem.id)}
                  >
                    Ротация
                  </button>
                  <button
      ...
      {tokenModalSecret ? (
        <div
          className="admin-token-modal-backdrop"
          role="dialog"
          aria-modal="true"
          aria-label="Новый сервисный токен"
        >
          <div className="card admin-token-modal">
            <h3 className="card-title">Ваш сервисный токен</h3>
            <p className="card-text">
              Вы увидите его только в этом окне. Сохраните значение сейчас.
            </p>
            <div className="admin-token-modal-row">
              <input
                className="ui-input admin-token-modal-input"
                value={tokenModalSecret}
                disabled
              />
            ...
            <div className="admin-token-modal-actions">
              <button
                type="button"
                className="btn primary"
                onClick={() => setTokenModalSecret(null)}
              >
                Закрыть
              </button>
            </div>
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>{item.reason ?? "—"}</td>
                      <td className="admin-audit-metadata" title={formatMetadata(item.metadata)}>
        ...
        {!user ? (
          ...
          <div className="admin-user-layout">
            <aside className="admin-user-side">
              ...
              <img
                className="admin-user-skin"
                src={buildSkinUrl(user.id, skinVersion)}
                alt={`Скин ${user.username}`}
              />
              <button
        ...
        {!squad ? (
          ...
          <>
            ...
            <dl className="admin-kv">
              <div>
                <dt>Создан</dt>
                <dd>{formatDateTime(squad.createdAt)}</dd>
              </div>
              <div>
                <dt>Обновлён</dt>
                <dd>{formatDateTime(squad.updatedAt)}</dd>
              </div>
              <div>
                <dt>Лидер</dt>
