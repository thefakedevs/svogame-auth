...
import { listAdminShopOrders, listAdminUsers, type AdminShopOrderResponse } from "../../api/admin";
...
export default function AdminTimelinePanel({ token }: { token: string }) {
  ...
  const lootboxTotalPages = Math.max(
    1,
    Math.ceil((lootboxHistory?.length ?? 0) / LOOTBOX_OPENINGS_PER_PAGE),
  );
    ...
    <div className="admin-shop-layout">
      <section className="card admin-card admin-shop-orders">
        ...
        <p className="admin-inline-muted admin-zero-margin">
          {orders
            ? `Всего покупок: ${ordersTotal}. Страница ${ordersPage} из ${ordersTotalPages}`
            : "Загружаем покупки..."}
        </p>
        ...
        {orders && orders.length === 0 ? (
          <p className="admin-inline-muted">Покупок пока нет.</p>
        ) : null}
        {orders && orders.length > 0 ? (
          <div className="admin-purchase-timeline">
            {orders.map(({ order, user }) => {
              ...
              return (
                <article
                  key={order.id}
                  className={`admin-purchase-card${isExpanded ? " is-expanded" : ""}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span
                      className={`ui-badge ${order.status === "fulfilled" ? "ui-badge-success" : order.status === "fulfillment_failed" ? "ui-badge-warning" : "ui-badge-neutral"}`}
                    >
                      {order.status}
                    </span>
                  </button>
                    ...
                    <div className="admin-purchase-details">
                      <dl className="admin-kv admin-purchase-kv">
                        <div>
                          <dt>Игрок</dt>
                          <dd>
                            <AdminLink href={adminUserPath(order.userId)}>
                              {user.username}
                            </AdminLink>
                          </dd>
                        </div>
                        ...
                        <div>
                          <dt>Asset</dt>
                          <dd>
                            {order.assetKey} · {order.ownershipModel}
                          </dd>
                        </div>
                        <div>
                          <dt>Количество</dt>
                        ...
                        <div>
                          <dt>Сумма</dt>
                          <dd>{formatRub(order.totalPriceRub)}</dd>
                        </div>
                        <div>
                          <dt>Оплата</dt>
                      ...
                      {order.failureProblem ? (
                        <p className="admin-inline-muted">Ошибка: {order.failureProblem}</p>
                      ) : null}
                        ...
                        <section>
                          ...
                          {order.receipt ? (
                            <dl className="admin-kv admin-purchase-kv">
                              <div>
                                <dt>ID</dt>
                                <dd>{order.receipt.id}</dd>
                              </div>
                              <div>
                                <dt>Статус</dt>
                                <dd>{order.receipt.displayStatus}</dd>
                              </div>
                              <div>
                                <dt>UUID</dt>
                                <dd>{order.receipt.receiptUuid ?? "—"}</dd>
                              </div>
                              <div>
                                <dt>Попытки</dt>
                                <dd>{order.receipt.attemptCount}</dd>
                              </div>
                            </dl>
                          ) : (
        ...
        {orders && orders.length > 0 ? (
          <AdminPagination
            ...
            label="Нумерация страниц покупок"
          />
        ) : null}
      </section>
      ...
      <section className="card admin-card admin-shop-orders">
        ...
        {!lootboxHistory && !lootboxError ? (
          <LoadingState title="Загружаем открытия кейсов" />
        ) : null}
        {lootboxHistory && lootboxHistory.length === 0 ? (
          <p className="admin-inline-muted">Кейсы пока не открывали.</p>
        ) : null}
        {lootboxHistory && lootboxHistory.length > 0 ? (
          <div className="admin-purchase-timeline">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`ui-badge ${item.wasCompensated ? "ui-badge-warning" : "ui-badge-success"}`}
                    >
                    ...
                    <div className="admin-purchase-details">
                      <dl className="admin-kv admin-purchase-kv">
                        <div>
                          <dt>Игрок</dt>
                          <dd>
                            <AdminLink href={adminUserPath(item.userId)}>{username}</AdminLink>
                          </dd>
                        </div>
                        <div>
                          <dt>Кейс</dt>
                        ...
                        <div>
                          <dt>Награда</dt>
                          <dd>{formatLootboxReward(item)}</dd>
                        </div>
                        <div>
                          <dt>Asset</dt>
                          <dd>
                            <code>{item.reward.assetKey}</code> · {item.reward.ownershipModel}
                          </dd>
                        </div>
                        <div>
                          <dt>Открыт</dt>
        ...
        {lootboxHistory && lootboxHistory.length > 0 ? (
          <AdminPagination
            page={lootboxPage}
            totalPages={lootboxTotalPages}
            onPageChange={setLootboxPage}
            label="Нумерация страниц открытий кейсов"
          />
        ) : null}
      </section>
    </div>
