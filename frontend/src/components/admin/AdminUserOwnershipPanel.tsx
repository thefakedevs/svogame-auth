...
function assetByKey(
  ...
  return (
    assetMap.get(assetKey) ?? (assetDefinitionId ? assetMap.get(assetDefinitionId) : null) ?? null
  );
}
...
export default function AdminUserOwnershipPanel({
  token,
  userId,
}: {
  token: string;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const walletTransactions = await getAdminUserWalletTransactions(
        token,
        userId,
        wallet.currencyKey,
      );
      ...
      setState({
        status: "error",
        error: toDisplayError(cause, "Не удалось загрузить инвентарь и кошелек."),
      });
    }
  }, [token, userId]);
  ...
  const entitlementAssets = useMemo(() => {
    ...
    return state.assets.filter(
      (asset) => !asset.isCurrency && asset.ownershipModel === "entitlement",
    );
  }, [state]);
  ...
  const stackableAssets = useMemo(() => {
    if (state.status !== "ready") return [];
    return state.assets.filter(
      (asset) => !asset.isCurrency && asset.ownershipModel === "stackable",
    );
  }, [state]);
  ...
  const expirableAssets = useMemo(() => {
    if (state.status !== "ready") return [];
    return state.assets.filter(
      (asset) => !asset.isCurrency && asset.ownershipModel === "expirable",
    );
  }, [state]);

  const selectedEntitlement =
    entitlementAssets.find((asset) => asset.key === selectedEntitlementKey) ??
    entitlementAssets[0] ??
    null;
  const selectedStackable =
    stackableAssets.find((asset) => asset.key === selectedStackableKey) ??
    stackableAssets[0] ??
    null;
  const selectedExpirable =
    expirableAssets.find((asset) => asset.key === selectedExpirableKey) ??
    expirableAssets[0] ??
    null;

  const inventoryMutationBody = (extra?: {
    amount?: number | null;
  ...
  const mutateWallet = async (kind: "credit" | "debit" | "adjust") => {
    ...
    if (amount === null || amount < 0 || (kind !== "adjust" && amount <= 0)) {
      toast.error(
        kind === "adjust" ? "Укажите неотрицательный баланс." : "Укажите положительную сумму.",
      );
      return;
    }
  ...
  const mutateEntitlement = async (action: "grant" | "revoke", assetKey?: string) => {
    ...
    await runMutation(
      async () => {
        if (action === "grant")
          return grantAdminEntitlement(token, userId, key, inventoryMutationBody());
  ...
  const mutateStackable = async (action: "add" | "remove" | "set" | "clear", assetKey?: string) => {
    ...
    if (amount === null || amount < 0 || (action !== "set" && action !== "clear" && amount <= 0)) {
      toast.error(
        action === "set"
          ? "Укажите неотрицательное количество."
    ...
    await runMutation(
      async () => {
        if (action === "add")
          return addAdminStackable(token, userId, key, inventoryMutationBody({ amount }));
        if (action === "remove")
          return removeAdminStackable(token, userId, key, inventoryMutationBody({ amount }));
  ...
  const mutateExpirable = async (
    action: "prolong" | "expiration" | "revoke",
    assetKey?: string,
  ) => {
      ...
      await runMutation(
        async () =>
          prolongAdminExpirable(
            ...
            userId,
            key,
            inventoryMutationBody({ durationSeconds: days * 24 * 60 * 60 }),
          ),
      ...
      await runMutation(
        async () =>
          setAdminExpirableExpiration(
            ...
            userId,
            key,
            inventoryMutationBody({ expiresAt: iso }),
          ),
    ...
    await runMutation(
      async () => revokeAdminExpirable(token, userId, key, inventoryMutationBody()),
      "Временный бонус снят.",
    );
  };
  ...
  const purchases = state.walletTransactions.filter(
    (transaction) => transaction.operationType.toLowerCase() === "purchase",
  );
    ...
    <section className="admin-card-subsection">
      ...
      <div className="admin-user-ownership-actions">
        ...
        <button
          type="button"
          className="btn btn-sm"
          onClick={() => setActiveModal("inventoryHistory")}
        >
      ...
      {activeModal === "wallet" ? (
        <AdminOwnershipModal
          title="Баланс игрока"
          titleId="admin-wallet-modal-title"
          isBusy={isMutating}
        ...
        >
          ...
          <div className="admin-ownership-form">
            ...
            <input
              className="ui-input"
              value={walletReasonText}
              onChange={(event) => setWalletReasonText(event.target.value)}
              placeholder="Причина"
            />
          </div>
          <div className="ui-modal-footer admin-wallet-modal-footer">
            ...
            <button
              ...
              type="button"
              disabled={isMutating}
              onClick={() => void mutateWallet("adjust")}
            >
              Выставить
            </button>
            <button
              ...
              type="button"
              disabled={isMutating}
              onClick={() => void mutateWallet("debit")}
            >
              Списать
            </button>
            <button
              ...
              type="button"
              disabled={isMutating}
              onClick={() => void mutateWallet("credit")}
            >
              Пополнить
            </button>
          </div>
      ...
      {activeModal === "entitlements" ? (
        <AdminOwnershipModal
          title="Скины игрока"
          titleId="admin-entitlements-modal-title"
          isBusy={isMutating}
          onClose={() => setActiveModal(null)}
        >
          <InventoryGrantForm
      ...
      {activeModal === "stackables" ? (
        <AdminOwnershipModal
          title="Предметы с количеством"
          titleId="admin-stackables-modal-title"
          isBusy={isMutating}
          onClose={() => setActiveModal(null)}
        >
      ...
      {activeModal === "expirables" ? (
        <AdminOwnershipModal
          ...
          titleId="admin-expirables-modal-title"
          isBusy={isMutating}
          onClose={() => setActiveModal(null)}
        >
      ...
      {activeModal === "purchases" ? (
        <AdminOwnershipModal
          ...
          titleId="admin-purchases-modal-title"
          isBusy={isMutating}
          onClose={() => setActiveModal(null)}
        >
      ...
      {activeModal === "inventoryHistory" ? (
        <AdminOwnershipModal
          title="История инвентаря"
          titleId="admin-inventory-history-modal-title"
          isBusy={isMutating}
          onClose={() => setActiveModal(null)}
        >
          <AdminHistoryModalList
  ...
  return (
    <AppPortal>
      <div className="ui-modal-backdrop" role="presentation" onClick={() => !isBusy && onClose()}>
        <div
          className="ui-modal admin-ownership-modal"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="ui-modal-body admin-ownership-modal-body">{children}</div>
        </div>
      </div>
    </AppPortal>
  );
...
function InventoryGrantForm({
  ...
  return (
    <div className="admin-ownership-form">
      ...
      />
      <div className="admin-ownership-actions">
        <button className="btn btn-sm primary" type="button" disabled={disabled} onClick={onGrant}>
          Выдать
        </button>
      </div>
    </div>
...
function StackableControlForm({
  ...
  if (!assets.length)
    return <p className="admin-inline-muted">Предметов с количеством в каталоге нет.</p>;
  ...
  return (
    <div className="admin-ownership-form">
      ...
      />
      <div className="admin-ownership-actions">
        <button className="btn btn-sm" type="button" disabled={disabled} onClick={onSet}>
          Выставить
        </button>
        <button className="btn btn-sm danger" type="button" disabled={disabled} onClick={onRemove}>
          Списать
        </button>
        <button className="btn btn-sm primary" type="button" disabled={disabled} onClick={onAdd}>
          Пополнить
        </button>
      </div>
    </div>
...
function ExpirableControlForm({
  ...
  if (!assets.length)
    return <p className="admin-inline-muted">Временных бонусов в каталоге нет.</p>;
  ...
  return (
    <div className="admin-ownership-form">
      ...
      />
      <div className="admin-ownership-actions">
        <button className="btn btn-sm" type="button" disabled={disabled} onClick={onSetExpiration}>
          Выставить срок
        </button>
        <button
      ...
      {items.length ? (
        <div className="admin-ownership-modal-list">
          {items.map((item) => (
            <article
              key={item.key}
              className="admin-ownership-list-item admin-ownership-list-item--actionable"
            >
...
function AdminHistoryModalList({
  items,
  emptyText,
}: {
