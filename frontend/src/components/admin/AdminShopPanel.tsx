...
function sortAdminShopProducts(
  list: ShopProductResponse[],
  sort: ShopAdminSortKey,
): ShopProductResponse[] {
  const out = [...list];
...
export default function AdminShopPanel({ token }: { token: string }) {
  ...
  const [productCatalogScope, setProductCatalogScope] = useState<"active_only" | "all">(
    "active_only",
  );
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled)
          setAssetsError(toDisplayError(cause, "Не удалось загрузить ассеты для выбора."));
      }
    };
    void run();
    return () => {
  ...
  const displayedProducts = useMemo(() => {
    ...
    const filtered = needle
      ? scoped.filter((p) =>
          `${p.key} ${p.assetKey} ${p.localizedName} ${p.localizedDescription ?? ""}`
            .toLowerCase()
            .includes(needle),
        )
      : scoped;
  ...
  const pagedProducts = displayedProducts.slice(
    (shopPage - 1) * SHOP_PRODUCTS_PER_PAGE,
    shopPage * SHOP_PRODUCTS_PER_PAGE,
  );
    ...
    <div className="admin-shop-layout">
      <section className="card admin-card admin-shop-list">
        <div className="admin-section-head">
          <div>
            ...
            <p className="card-text">
              Товары витрины, цены, локали, расписание доступности и привязанные ассеты.
            </p>
          </div>
        ...
        <div className="admin-shop-list-controls admin-filter-bar">
          <div className="admin-shop-list-controls-row">
            ...
            <label className="admin-shop-field admin-filter-label">
              ...
              <select
                className="ui-input"
                value={shopSort}
                onChange={(e) => setShopSort(e.target.value as ShopAdminSortKey)}
          ...
          {listCountLabel ? (
            <p className="admin-inline-muted admin-zero-margin">{listCountLabel}</p>
          ) : null}
        </div>
        ...
        {items && items.length === 0 ? (
          <p className="admin-inline-muted">Товаров пока нет.</p>
        ) : null}
        {items && items.length > 0 && displayedProducts.length === 0 ? (
          <p className="admin-inline-muted">
            Нет активных товаров. Включите «Все товары», чтобы увидеть выключенные позиции.
          </p>
        ) : null}
            ...
            {pagedProducts.map((p) => (
              <AdminLink key={p.id} href={adminShopProductPath(p.id)} className="admin-row">
                ...
                <div className="admin-row-badges">
                  <span
                    className={`ui-badge ${p.isActive ? "ui-badge-success" : "ui-badge-warning"}`}
                  >
                    {p.isActive ? "Активен" : "Выкл"}
                  </span>
                  <span
                    className={`ui-badge ${p.isPublic ? "ui-badge-neutral" : "ui-badge-secondary"}`}
                  >
                    {p.isPublic ? "Публичный" : "Скрыт"}
                  </span>
                </div>
              </AdminLink>
            ))}
        ...
        {displayedProducts.length > 0 ? (
          <AdminPagination
            ...
            label="Нумерация страниц товаров"
          />
        ) : null}
      </section>
      ...
      {createModalOpen ? (
        <AppPortal>
          ...
          >
            <div
              className="ui-modal admin-create-modal"
              role="dialog"
              aria-modal="true"
              aria-labelledby="admin-create-shop-title"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="ui-modal-header">
                <h2 id="admin-create-shop-title" className="ui-modal-title">
                  Новый товар
                </h2>
                <button
                  type="button"
              ...
              <div className="ui-modal-body">
                <p className="card-text admin-inline-muted admin-modal-note">
                  Выберите ассет, и форма покажет только поля, которые принимает его модель
                  владения.
                </p>
                <div className="admin-shop-form-grid">
                  ...
                  <label className="admin-shop-field admin-shop-field--asset-select">
                    ...
                    {!assetsError && catalogAssets && pickableAssets.length === 0 ? (
                      <span className="admin-inline-muted">
                        Нет свободных невалютных ассетов: все уже привязаны к товарам или
                    ...
                    <>
                      <label className="admin-shop-field">
                        ...
                        <input
                          ...
                          type="number"
                          min={1}
                          value={stackableAmount}
                          onChange={(e) => setStackableAmount(e.target.value)}
                          placeholder="например 10"
                        />
                        <small>
                          Обязательное поле: сколько единиц stackable-ассета начисляется за одну
                          покупку.
                        </small>
                      </label>
                      <label className="admin-shop-field">
                      ...
                      <label className="admin-shop-field">
                        ...
                        <input
                          ...
                          type="number"
                          min={1}
                          value={maxOwnedAmount}
                          onChange={(e) => setMaxOwnedAmount(e.target.value)}
                          placeholder="опционально"
                        />
                        <small>
                          Только для stackable: максимум единиц ассета у игрока после покупки.
                        </small>
                      </label>
                    </>
                  ) : null}
                  ...
                  {selectedAsset?.ownershipModel === "entitlement" ? (
                    <p className="admin-field-hint admin-shop-field--full">
                      Для entitlement-товара дополнительных ownership-полей нет: сервер принимает
                      только цену, локали, флаги и расписание.
                    </p>
                  ) : null}
                  <label className="admin-shop-field">
                  ...
                  {localeRows.map((row, index) => (
                    <div key={index} className="admin-shop-locale-row">
                      <input
                        className="ui-input"
                        value={row.locale}
                        onChange={(e) => updateLocaleRow(index, { locale: e.target.value })}
                        placeholder="ru-RU"
                        aria-label="Код локали"
                      />
                      <input
                        className="ui-input"
                        value={row.name}
                        onChange={(e) => updateLocaleRow(index, { name: e.target.value })}
                        placeholder="Название"
                      />
                      <textarea
                        className="ui-input"
                        value={row.description}
                        onChange={(e) => updateLocaleRow(index, { description: e.target.value })}
                        placeholder="Описание"
                      />
                      <button
                        type="button"
                        className="btn btn-sm danger"
                        disabled={localeRows.length <= 1}
                        onClick={() => removeLocaleRow(index)}
                      >
                        Удалить локаль
                      </button>
                    </div>
