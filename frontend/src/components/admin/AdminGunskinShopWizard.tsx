  ...
  return (
    <section className="card admin-card admin-skin-wizard" aria-label="Новый скин и товар">
      ...
      <p className="admin-inline-muted">
        Создаётся ассет вида <code>skin</code> с моделью владения <code>entitlement</code>, загружается превью, затем
        привязывается товар магазина. По умолчанию товар <strong>не публичный</strong> в витрине магазина, ассет — публичный
        в каталоге (если не снять флажок).
      </p>
      ...
      <div className="admin-asset-form-grid">
        ...
        />
        <select className="ui-input" value={rarity} onChange={(e) => setRarity(e.target.value as SkinRarity)}>
      ...
      <div className="admin-asset-flags">
        <label className="admin-checkbox-row">
          <input type="checkbox" checked={assetActive} onChange={(e) => setAssetActive(e.target.checked)} />
          Ассет активен
        </label>
        <label className="admin-checkbox-row">
          <input type="checkbox" checked={assetPublic} onChange={(e) => setAssetPublic(e.target.checked)} />
      ...
      <div className="admin-asset-flags">
        <label className="admin-checkbox-row">
          <input type="checkbox" checked={shopActive} onChange={(e) => setShopActive(e.target.checked)} />
          Товар активен
        </label>
        <label className="admin-checkbox-row">
          <input type="checkbox" checked={shopPublic} onChange={(e) => setShopPublic(e.target.checked)} />
          Товар публичный в магазине (витрина)
        </label>
      </div>

      <div className="admin-skin-wizard__actions">
        <button type="button" className="btn primary" disabled={isSubmitting} onClick={() => void submit()}>
