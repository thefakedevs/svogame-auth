  ...
  return (
    <div className="admin-page">
      ...
      {squad && isDeleteAvatarModalOpen ? (
        <AppPortal>
          <div className="ui-modal-backdrop" role="presentation" onClick={() => !isDeletingAvatar && setIsDeleteAvatarModalOpen(false)}>
            <div className="ui-modal" role="dialog" aria-modal="true" aria-labelledby="delete-squad-avatar-modal-title" onClick={(event) => event.stopPropagation()}>
              <div className="ui-modal-header">
                <h2 id="delete-squad-avatar-modal-title" className="ui-modal-title">Удалить аватарку сквада?</h2>
                <button className="ui-modal-close" type="button" aria-label="Закрыть" onClick={() => setIsDeleteAvatarModalOpen(false)} disabled={isDeletingAvatar}>
                  ×
                </button>
              </div>
              <div className="ui-modal-body">
                <p>Аватарка сквада <strong>{squad.name}</strong> будет удалена. Подтвердите действие.</p>
              </div>
              <div className="ui-modal-footer">
                <button className="btn" type="button" onClick={() => setIsDeleteAvatarModalOpen(false)} disabled={isDeletingAvatar}>
                  Отменить
                </button>
      ...
      {squad && isDeleteSquadModalOpen ? (
        <AppPortal>
          <div className="ui-modal-backdrop" role="presentation" onClick={() => !isDeletingSquad && setIsDeleteSquadModalOpen(false)}>
            <div className="ui-modal" role="dialog" aria-modal="true" aria-labelledby="delete-squad-modal-title" onClick={(event) => event.stopPropagation()}>
              <div className="ui-modal-header">
                <h2 id="delete-squad-modal-title" className="ui-modal-title">Удалить сквад?</h2>
                <button className="ui-modal-close" type="button" aria-label="Закрыть" onClick={() => setIsDeleteSquadModalOpen(false)} disabled={isDeletingSquad}>
                  ×
                </button>
              </div>
              <div className="ui-modal-body">
                <p>
                  Сквад <strong>{squad.name}</strong> будет удален без возможности восстановления. Подтвердите действие.
                </p>
              </div>
              <div className="ui-modal-footer">
                <button className="btn" type="button" onClick={() => setIsDeleteSquadModalOpen(false)} disabled={isDeletingSquad}>
                  Отменить
                </button>
                <button className="btn danger" type="button" onClick={() => void removeSquad()} disabled={isDeletingSquad}>
