  ...
  const visibleItems = useMemo(() => {
    ...
    const filtered = needle
      ? state.items.filter((item) =>
          `${item.assetKey} ${item.assetDisplayName} ${item.assetDescription ?? ""}`
            .toLowerCase()
            .includes(needle),
        )
      : state.items;
          ...
          {state.status === "ready" ? (
            <>
              <p className="admin-inline-muted">
                Показано: {visibleItems.length} из {state.items.length}. Страница {page} из{" "}
                {totalPages}
              </p>
              <div className="admin-list">
                {pagedItems.map((item) => (
                  <AdminLink key={item.id} className="admin-row" href={adminLootboxPath(item.id)}>
                    <span className="admin-row-user-text">
                      <strong>{item.assetDisplayName}</strong>
                      <small>
                        {item.assetKey} · {item.id}
                      </small>
                    </span>
                    <span className="admin-row-badges">
                      <span
                        className={`ui-badge ${item.isActive ? "ui-badge-success" : "ui-badge-warning"}`}
                      >
                        {item.isActive ? "active" : "inactive"}
                      </span>
                      <span
                        className={`ui-badge ${item.isPublic ? "ui-badge-secondary" : "ui-badge-neutral"}`}
                      >
                ...
                {!visibleItems.length ? (
                  <p className="admin-inline-muted">Лутбоксы не найдены.</p>
                ) : null}
              </div>
              {visibleItems.length ? (
