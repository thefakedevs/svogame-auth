...
export default function AdminLittlemiceTimelineView({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled)
          setError(toDisplayError(cause, "Не удалось загрузить общий таймлайн littlemice."));
      }
    };
    void run();
    return () => {
  ...
  useEffect(() => {
    if (!response) return;
    const missingIds = Array.from(new Set(response.items.map((item) => item.playerUuid))).filter(
      (id) => !(id in usersById),
    );
        ...
        <div className="admin-littlemice-head">
          <div>
            ...
            <p className="card-text">
              Последние проверки по всем игрокам. Карточка открывает конкретную проверку, имя игрока
              ведет в его историю.
            </p>
          </div>
          {response ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="admin-littlemice-timeline-main">
                        <span className="admin-littlemice-timeline-head">
                          <span className="admin-littlemice-identity">
                            <AdminLink
                              ...
                              href={adminUserLittlemicePath(item.playerUuid)}
                            >
                        ...
                        {item.failureReason ? (
                          <span className="admin-littlemice-failure">
                            Причина: {item.failureReason}
                          </span>
                        ) : null}
                      </span>
                    </article>
