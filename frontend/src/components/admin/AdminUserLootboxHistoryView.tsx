...
export default function AdminUserLootboxHistoryView({ token, userId }: { token: string; userId: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled) setError(toDisplayError(cause, 'Не удалось загрузить историю открытий кейсов.'))
      }
    }
    void run()
    return () => {
              ...
              <table className="admin-audit-table">
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id}>
                      <td>{item.id}</td>
                      <td><code>{item.lootboxAssetKey}</code></td>
                      <td>
                        <strong>{formatReward(item)}</strong>
                        <br />
                        <small><code>{item.reward.assetKey}</code>{item.reward.title ? ` · ${item.reward.title}` : ''}</small>
                      </td>
                      <td>{item.reward.ownershipModel}</td>
                      <td>{item.actorKind}{item.actorServiceName ? ` · ${item.actorServiceName}` : ''}</td>
                      <td>{item.winnerIndex + 1}/{item.feedLength}</td>
