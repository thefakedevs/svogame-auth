...
function usesAttachmentAdapterAsDefaultStock(modelIdentifier: string) {
  ...
  return (
    normalized.includes("m4a1") || normalized.includes("rpk") || normalized.includes("sks_tactical")
  );
}
...
function resolvePreviewVisibleBones(bones: GeckoBone[], modelIdentifier: string) {
  ...
  const isVisible = (bone: GeckoBone): boolean => {
    ...
    const visible =
      !isHiddenPreviewBoneName(bone.name, modelIdentifier) && (!parent || isVisible(parent));
...
function convertBonePivot(bone: GeckoBone, parent?: GeckoBone): Vec3 {
  ...
  return [pivot[0] - parentPivot[0], parentPivot[1] - pivot[1], pivot[2] - parentPivot[2]];
}
...
function smoothSharedVertexNormals(geometry: THREE.BufferGeometry) {
  ...
  if (!(positions instanceof THREE.BufferAttribute) || !(normals instanceof THREE.BufferAttribute))
    return;
      ...
      for (const bone of bones) {
        ...
        for (const cube of bone.cubes ?? []) {
          const cubeTransform = cube.rotation
            ? boneTransform
                .clone()
                .multiply(makePartMatrix(convertCubePivot(bone, cube), cube.rotation))
            : boneTransform;
          const cubeOrigin = cube.rotation
            ? convertRotatedCubeOrigin(cube)
            : convertCubeOrigin(bone, cube);
          pushCube(
            cube,
            cubeOrigin,
            cubeTransform,
...
export default function GeckoModelViewer({
  ...
  return (
    <div ref={rootRef} className={classes}>
      ...
      {!error && (
        <div className="gecko-model-viewer__hint">Потяните влево или вправо, чтобы повернуть</div>
      )}
      {isLoading && <div className="gecko-model-viewer__state">Loading model</div>}
      {error && (
        <div className="gecko-model-viewer__state gecko-model-viewer__state_error">{error}</div>
      )}
    </div>
  );
}
