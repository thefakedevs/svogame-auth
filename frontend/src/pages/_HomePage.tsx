...
function pickLandingScheduleEvents(
  events: DiscordGuildEventResponse[],
): DiscordGuildEventResponse[] {
  const now = Date.now();
    ...
    >
      <div className="landing-shop-ticker-card__visual">
        <div className="landing-shop-ticker-card__overlay">
          ...
          <span className="landing-shop-ticker-card__rarity">
            {rarityLabel(item.asset?.rarity)}
          </span>
        </div>
        <InventoryVisual item={item} />
      </div>
      <div className="landing-shop-ticker-card__footer">
        <span className="landing-shop-ticker-card__price">
...
export default function HomePage() {
  ...
  const [discordEventsState, setDiscordEventsState] = useState<DiscordEventsState>({
    status: "loading",
  });
  ...
  const scheduleEvents =
    discordEventsState.status === "ready"
  ...
  const productViews = useMemo(() => {
    ...
    return shopState.products
      .map((product) => {
        const asset =
          assetMap.get(product.assetKey) ?? assetMap.get(product.assetDefinitionId) ?? null;
        const title =
          product.localizedName || asset?.displayName || product.assetDisplayName || product.key;
  ...
  useEffect(() => {
    ...
    Promise.all([listPublicShopProducts("ru-RU"), listAllEntitlementAssets()])
  ...
  return (
    <main className="page home-page landing-page">
      <section className="landing-hero" aria-labelledby="landing-hero-title">
        ...
        <div className="landing-hero__content">
          <h1 id="landing-hero-title" className="landing-hero__title">
            Зайди в бой за две минуты
          </h1>
          <p className="landing-hero__lead">
            Командные ивенты, быстрые мини-игры и вход без лишней настройки. Скачайте лаунчер,
            авторизуйтесь через Discord и заходите в матч, пока арены горячие.
          </p>
      ...
      <section
        className="landing-section flex flex-col gap-4"
        aria-labelledby="landing-modes-title"
      >
        <div className="landing-section-head">
          <span className="ui-section-title">Режимы</span>
          <h2 id="landing-modes-title" className="landing-section-head__title">
            Матчи под разный темп
          </h2>
        </div>
        <div className="landing-mode-grid">
          <article className="card landing-mode-card">
            ...
            <p className="card-text">
              Крупные события с участием большого количества игроков, активной модерацией и
              проработанными системами. Является приоритетным и самым ожидаемым режимом среди
              игроков.
            </p>
          ...
          <article className="card landing-mode-card">
            ...
            <p className="card-text">
              Небольшие режимы доступные абсолютно всегда. Отлично подходят для игры в свободное
              время с своими друзьями, а также для знакомства с механиками и оттачивания навыков.
            </p>
            <div className="ui-card-footer">
              <a
                className="btn btn-sm btn-secondary-accent"
      ...
      <section className="landing-section landing-start" aria-labelledby="landing-start-title">
        <div className="landing-section-head">
          ...
          <h2 id="landing-start-title" className="landing-section-head__title">
            Четыре шага до первого матча
          </h2>
          <p className="landing-section-head__text">
            Лаунчер сам держит клиент в актуальном состоянии, а вход через Discord привязывает
            профиль к аккаунту SvoCraft.
          </p>
        </div>
      ...
      <section id="gallery" className="landing-section" aria-labelledby="landing-gallery-title">
        <div className="landing-section-head">
          ...
          <h2 id="landing-gallery-title" className="landing-section-head__title">
            Галерея
          </h2>
          <p className="landing-section-head__text">
            Превью арен, мини-игр и лаунчера. Нажмите на кадр, чтобы открыть изображение на весь
            экран.
          </p>
        </div>
            ...
            <PhotoView
              ...
              overlay={
                <div className="landing-photo-overlay">
                  ...
                  <span>
                    Карта для событий с упором на манёвры, спроектированная с вниманием к деталям,
                    специально под режим, который на ней проводится.
                  </span>
                </div>
              }
            >
              ...
              >
                ...
                <span className="landing-gallery-card__caption">
                  ...
                  <small>
                    Карта для событий с упором на манёвры, спроектированная с вниманием к деталям,
                    под режим, который на ней проводится.
                  </small>
                </span>
              </button>
            </PhotoView>
            ...
            <PhotoView
              ...
              overlay={
                <div className="landing-photo-overlay">
                  ...
                  <span>
                    Минималистичная арена, где исход решают точность, тайминг и хладнокровие,
                    разработанная специально для того, чтобы позволить показать свой скилл.
                  </span>
                </div>
              }
            >
            ...
            <PhotoView
              ...
              overlay={
                <div className="landing-photo-overlay">
                  <strong>Лобби</strong>
                  <span>
                    Безопасная точка старта, где игроки собирают отряд перед матчем, формируют
                    отряды, готовятся к бою и выбирают дальнейший путь.
                  </span>
                </div>
              }
            >
            ...
            <PhotoView
              ...
              overlay={
                <div className="landing-photo-overlay">
                  ...
                  <span>
                    Современный центр управления игрой, через который осуществляется вход, загрузка
                    обновлений и быстрый запуск клиента без лишних действий.
                  </span>
                </div>
              }
            >
      ...
      <section id="shop" className="landing-section" aria-labelledby="landing-shop-title">
        <div className="landing-section-head">
          ...
          <h2 id="landing-shop-title" className="landing-section-head__title">
            Покупайте косметику и поддерживайте сервер
          </h2>
          <p className="landing-section-head__text">
            В нашем магазине вы найдете эксклюзивные косметические предметы, которые не только
            украсят ваш игровой опыт, но и помогут поддержать развитие сервера.
          </p>
        </div>
        <div className="landing-shop-grid">
          ...
          {shopState.status === "ready" && productViews.length > 0 && (
            <div
              className="landing-shop-ticker"
              aria-live="polite"
              aria-label="Популярные товары магазина"
            >
              <div ref={ref} className="landing-shop-ticker__track">
                {productViews.map((item) => (
                  <ShopTickerCard key={`original-${item.product.id}`} item={item} />
                ))}

                {productViews.map((item) => (
                  <ShopTickerCard key={`duplicate-${item.product.id}`} item={item} isDuplicate />
                ))}
              </div>
            </div>
          )}
      ...
      <section id="community" className="landing-section" aria-labelledby="landing-community-title">
        <div className="landing-section-head">
          ...
          <h2 id="landing-community-title" className="landing-section-head__title">
            Не заходите в пустоту
          </h2>
          <p className="landing-section-head__text">
            Игроки держат темп в мини-играх, собираются на ивенты и помогают новичкам быстрее
            попасть в первый матч.
          </p>
        </div>
        <div className="landing-community-grid">
          <div className="landing-community-hero">
            ...
            <div
              className="card landing-events-card"
              aria-live="polite"
              aria-label="Расписание игр из Discord"
            >
                    ...
                    <li key={ev.id} className="landing-events-card__row">
                      <div className="landing-events-card__meta">
                        <time dateTime={ev.startsAt}>
                          {landingEventDateFormatter.format(new Date(ev.startsAt))}
                        </time>
                        <span
                          className={
                            ev.status === "active"
                      ...
                      <div className="landing-events-card__body">
                        ...
                        {typeof ev.userCount === "number" && ev.userCount > 0 ? (
                          <span className="landing-events-card__count">{ev.userCount} участ.</span>
                        ) : null}
                      </div>
                    </li>
                  ))}
          ...
          <div className="landing-testimonials">
            <blockquote className="card landing-testimonial">
              <p>
                Зашёл на сервер, сначала было немного непривычно, но быстро втянулся, лёг на
                балконе, взял монитор в руки и начал управление дроном, пару дронов, и вражеская
                пума подбита, на вырученные средства прикупил себе новые зимние ботинки, теперь
                питаюсь только стейками, прикупил себе новенький AWM, которым отстреливаю различную
                перхоть на своей, немного побитой терассе, попивая чашечку кофе.
              </p>
              <footer>
                <strong>Eblan</strong>
            ...
            <blockquote className="card landing-testimonial">
              <p>
                SvoCraft — Отличный сервер для тех, кто любит PvP. Захват флага держит в напряжении
                до последней секунды, а дуэли — лучший способ проверить свою реакцию. Заходите, не
                пожалеете.
              </p>
              <footer>
      ...
      <section className="landing-final-cta" aria-labelledby="landing-final-title">
        <div className="landing-final-cta__content">
          ...
          <p>
            Постоянные мини-игры держат форму, а ивентовые игры дважды в неделю дает цель для
            сквада. Чем раньше зайдете, тем быстрее привыкнете к аренам и таймингам.
          </p>
        </div>
