...
export default function UiKitPage() {
  ...
  const onUploadDrop = (event: React.DragEvent) => {
    ...
    if (files?.length) {
      setUploadFileHint(Array.from(files).map((file) => file.name).join(', '))
    }
  }
    ...
    <>
      ...
      <div className="page ui-kit-page">
        <header className="ui-kit-hero">
          <h1>UI kit</h1>
          <p className="ui-muted">Острые углы, оранжево-синий акцент, карточки #19191C, размытие 3px и VHS-фон.</p>
        </header>
        ...
        <section className="ui-section" aria-labelledby="section-upload">
          <h2 id="section-upload" className="ui-section-title">Upload</h2>
          <div
          ...
          >
            ...
            <label htmlFor="ui-kit-upload" className="ui-upload-label">
              <span className="ui-upload-icon" aria-hidden>⬆</span>
        ...
        <section className="ui-section" aria-labelledby="section-pagination">
          <h2 id="section-pagination" className="ui-section-title">Pagination</h2>
          <nav aria-label="Нумерация страниц">
            <ul className="ui-pagination">
              <li>
                <button type="button" className="ui-pagination-btn" aria-label="Предыдущая страница" disabled={paginationPage <= 1} onClick={() => setPaginationPage((page) => Math.max(1, page - 1))}>‹</button>
              </li>
              ...
              <li>
                <button type="button" className="ui-pagination-btn" aria-label="Следующая страница" disabled={paginationPage >= paginationTotal} onClick={() => setPaginationPage((page) => Math.min(paginationTotal, page + 1))}>›</button>
              </li>
            </ul>
          </nav>
        ...
        <section className="ui-section" aria-labelledby="section-modal">
          <h2 id="section-modal" className="ui-section-title">Modal</h2>
        ...
        {modalOpen ? (
          <div className="ui-modal-backdrop" role="presentation" onClick={() => setModalOpen(false)}>
            <div className="ui-modal" role="dialog" aria-modal="true" aria-labelledby="ui-kit-modal-title" onClick={(event) => event.stopPropagation()}>
              <div className="ui-modal-header">
                <h2 id="ui-kit-modal-title" className="ui-modal-title">Заголовок окна</h2>
                <button type="button" className="ui-modal-close" aria-label="Закрыть" onClick={() => setModalOpen(false)}>×</button>
              </div>
              <div className="ui-modal-body">
                Содержимое модального окна. Клик по затемнению или Escape закрывает окно. Для продуктового режима стоит добавить ловушку фокуса.
              </div>
              <div className="ui-modal-footer">
                <button type="button" className="btn btn-sm" onClick={() => setModalOpen(false)}>Отмена</button>
                <button type="button" className="btn primary btn-sm" onClick={() => setModalOpen(false)}>Ок</button>
              </div>
            </div>
          </div>
        ) : null}
