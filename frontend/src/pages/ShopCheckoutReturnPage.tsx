      ...
      <div className="shop-receipt-panel">
        ...
        <div className="shop-callback-actions">
          <a
            ...
            href={state.imageUrl}
            download={`receipt-${state.receipt.id}.png`}
          >
            Скачать чек
          </a>
...
function CallbackActions() {
  return (
    <div className="shop-callback-actions">
      <a className="btn primary" href={paths.shop}>
        Продолжить покупки
      </a>
      <a className="btn" href={paths.inventory}>
        Перейти в инвентарь
      </a>
    </div>
  );
...
export default function ShopCheckoutReturnPage() {
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled) {
          setState({
            status: "error",
            error: toDisplayError(cause, "Не удалось проверить заказ."),
          });
        }
      }
    };
          ...
          try {
            ...
            setReceiptState({
              ...
              message: toDisplayError(
                cause,
                "Чек готов, но изображение для печати пока недоступно.",
              ),
            });
          }
          return;
    ...
    return (
      <main className="page ownership-page shop-page">
        <ErrorState
          ...
          message="В ссылке возврата нет orderId."
          primaryActionLabel="В магазин"
          onPrimaryAction={() => window.location.assign(paths.shop)}
        />
      </main>
    );
  }
  ...
  if (state.status === "error") {
    return (
      <main className="page ownership-page shop-page">
        <ErrorState
          title="Статус заказа недоступен"
          message={state.error}
  ...
  return (
    <main className="page ownership-page shop-page">
      <LoadingState
        ...
        message={state.order ? statusText(state.order.status) : "Запрашиваем статус заказа."}
      >
