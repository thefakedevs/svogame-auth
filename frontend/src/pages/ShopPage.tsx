...
type ShopState =
  ...
  | {
      ...
      lootboxes: LootboxDefinitionResponse[];
    };
...
function isSkinProduct(product: ShopProductResponse, asset: AssetResponse | null) {
  return (
    normalizedAssetKind(product, asset) === "skin" || Boolean(asset?.weaponKey || asset?.rarity)
  );
}
...
function buildProductMetaItems(product: ShopProductResponse, asset: AssetResponse | null) {
  ...
  if (ownershipModel === "stackable") {
    addMetaItem(
      ...
      product.stackableAmount ? String(product.stackableAmount) : null,
    );
    addMetaItem(
      rows,
      "Максимум за покупку",
      product.maxPerPurchase ? String(product.maxPerPurchase) : null,
    );
  }
    ...
    <AppPortal>
      <div className="ui-modal-backdrop" role="presentation" onClick={onClose}>
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="shop-lootbox-details-title"
          onClick={(event) => event.stopPropagation()}
        >
          <div className="ui-modal-header">
            <h2 id="shop-lootbox-details-title" className="ui-modal-title">
              {item.title}
            </h2>
                ...
                <section className="lootbox-details-modal__drops" aria-label="Содержимое кейса">
                  ...
                  {drops.length ? (
                    <div className="lootbox-details-modal__drop-list">
                      {drops.map((drop) => {
                        const reward = productRewardView(
                          assetMap,
                          drop.rewardAssetKey,
...
export default function ShopPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [products, assets, entitlements, lootboxes] = await Promise.all([
        ...
        listAllPublicAssets(),
        token
          ? listMyEntitlements(token).catch(() => [] as EntitlementResponse[])
          : Promise.resolve([] as EntitlementResponse[]),
  ...
  const baseProductViews = useMemo(() => {
    ...
    return state.products.map((product) => {
      ...
      const lootbox =
        lootboxMap.get(product.assetKey) ?? lootboxMap.get(product.assetDefinitionId) ?? null;
      const title =
  ...
  const productViews = useMemo(() => {
    ...
    if (shopAssetKindFilter !== "all") {
      rows = rows.filter(
        (item) => normalizedAssetKind(item.product, item.asset) === shopAssetKindFilter,
      );
    }
    ...
    const sorted = [...rows];
    const tieTitle = (a: ShopProductView, b: ShopProductView) =>
    ...
    switch (shopSort) {
      ...
      case "rarity":
        sorted.sort(
          (a, b) =>
            skinRarityRank(a.asset?.rarity) - skinRarityRank(b.asset?.rarity) || tieTitle(a, b),
        );
        break;
      case "weapon": {
  ...
  const confirmTotalPrice = confirmProduct
    ? confirmProduct.product.priceRub * normalizedConfirmQuantity
    : 0;
    ...
    <main className="page ownership-page shop-page">
      ...
      {isShopFiltersModalOpen ? (
        <AppPortal>
          ...
          >
            ...
            >
              <div className="ui-modal-header">
                <h2 id="shop-filters-modal-title" className="ui-modal-title">
              ...
              <div className="ui-modal-body inventory-filters-modal__body">
                <div
                  ...
                  aria-label="Сортировка и фильтры магазина"
                >
      ...
      {detailsProduct ? (
        <SkinDetailsModal
          ...
          footer={
            <>
              <button className="btn" type="button" onClick={() => setDetailsProduct(null)}>
                Закрыть
              </button>
      ...
      {confirmProduct ? (
        ...
        >
          <div className="shop-confirm">
            <p className="shop-confirm__title">Купить «{confirmProduct.title}»</p>
                ...
                <div className="shop-confirm__quantity-row">
                  <div className="shop-confirm__quantity-control">
                    <button
                      ...
                      onClick={() =>
                        setConfirmQuantity((value) =>
                          clampPurchaseQuantity(confirmProduct, value - 1),
                        )
                      }
                    >
                      -
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setConfirmQuantity(
                          clampPurchaseQuantity(confirmProduct, Number(event.target.value)),
                        )
                      }
                    />
                    <button
                  ...
                  <div className="shop-confirm__presets" aria-label="Быстрый выбор количества">
                    {confirmQuantityPresets.map((preset) => (
                      <button
                        ...
                        onClick={() =>
                          setConfirmQuantity(clampPurchaseQuantity(confirmProduct, preset))
                        }
                      >
                        x{preset}
                      </button>
